...
function processFiles(files) {
    ...
    if (uploadedFiles.length + files.length > CONFIG.MAX_FILES) {
...
function displayFilePreviews() {
    ...
    previewContainer.innerHTML = uploadedFiles.map(fileObj => `
...
function removeFile(fileId) {
    uploadedFiles = uploadedFiles.filter(f => f.id.toString() !== fileId.toString());
    displayFilePreviews();
}
...
async function sendMessage() {
    ...
    try {
        ...
        // Build prompt
        const prompt = buildPrompt(message, uploadedFiles);
...
async function processUploadedFiles() {
    ...
    const promises = uploadedFiles.map(async (fileObj) => {
...
function displayUserMessage(message, files) {
    ...
    if (files.length > 0) {
        ...
        files.forEach(fileObj => {
            ...
            if (fileObj.preview && fileObj.type.startsWith('image/')) {
                fileCard.innerHTML = `
          <img src="${fileObj.preview}" alt="${fileObj.name}" class="file-card-image">
        `;
            } else {
                fileCard.innerHTML = `
          <div class="file-card-icon">${getFileIcon(fileObj.type)}</div>
          <div class="file-card-info">
            <div class="file-card-name">${fileObj.name}</div>
            <div class="file-card-size">${formatFileSize(fileObj.size)}</div>
          </div>
        `;
            }
...
function displayAIMessage(content) {
    ...
    const messageWrapper = document.createElement('div');
    messageWrapper.className = 'message-wrapper ai-message-wrapper';
...
function saveToHistory(message) {
    ...
    sessionStorage.setItem('mindx_conversation', JSON.stringify(conversationHistory));
}
